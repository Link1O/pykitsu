...
from pykitsu.utils import _RequestLimiter, get_latest_id
from pykitsu.exceptions import *
from pykitsu.value_errors import *
class Client:
    class search:
        def __init__(self, type: str, search_term: str, title_type: str = "en_jp", poster_size: str = "medium", limit_requests: bool = False, debug_outputs: bool = False):
            """
            ...
            if self.title_type != "en_jp":
                if self.title_type != "en":
                    if self.title_type != "ja_jp":
                        raise INVALID_ARGUMENT("title type")
            self.poster_size = poster_size
            if self.poster_size != "medium":
                if self.poster_size != "small":
                    if self.poster_size != "large": 
                        if self.poster_size != "tiny":
                            if self.poster_size != "original":
        ...
        async def id(self, offset: int = 0):
            ...
            self.cache_id[self.cache_key] = id
            return id
        async def name(self, offset: int = 0):
            """
        ...
        async def plot(self, offset: int = 0):
            ...
            return plot
        async def poster_url(self, offset: int = 0):
            """
    ...
    class random:
        ...
        async def _fetch_random(self):
            if self.limit_requests:
                rand_int = random.randint(1, await get_latest_id(type=self.type))
            else:
